...
def answer(s):
    new_s = []
    aclauses = s.split('+')
    for aclause in aclauses:
        mclauses = aclause.split('*')
        for mclause in mclauses:
            new_s.append(mclause)
        for _ in range(len(mclauses)-1):
            new_s.append('*')
    for _ in range(len(aclauses)-1):
        new_s.append('+')
    return ''.join(new_s)

if __name__=='__main__':
    for s in ['2+3*2', '2*4*3+9*3+5', '', '2']:
        print '%s ::: %s' % (s, answer(s))
